...
my $crc = 0x0000;  # Initial value of CRC
open(my $fh, '<', 'parsed/pat_signatures.txt') or die "Cannot open file: $!";
while (my $line = <$fh>) {
    chomp $line;  # Remove the newline character at the end of the line
    $crc = crc16($line, $crc);  # Calculate CRC for the line
    print "CRC for $line is: $crc\n";  # Print the CRC for the line
}
close($fh);